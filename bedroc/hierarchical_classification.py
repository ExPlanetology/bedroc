...
class GroupClassifierModel:
    ...
    def plot_group_fraction_posterior(
        ...
        *,
        ...
        The posterior is shown together with the Beta prior and, when available, the observed group
        fraction.

        Args:
            result: Result returned by :meth:`infer_group_fraction`
        ...
        fraction_0_samples: NpFloat = result["fraction_0_samples"]
        fraction_1_samples: NpFloat = result["fraction_1_samples"]
        kde_0 = gaussian_kde(fraction_0_samples)
        kde_1 = gaussian_kde(fraction_1_samples)
        ...
        x: NpFloat = np.linspace(0, 1, 1000)
        ...
        ax.plot(x, kde_0(x), color=color_0, linewidth=2, label=f"{group_0}")
        ax.hist(
            fraction_1_samples,
            bins=bins,
            density=True,
        ...
        ax.plot(x, kde_1(x), color=color_1, linewidth=2, label=f"{group_1}")

        # Beta prior
        prior_pdf: NpArray = beta.pdf(x, prior_alpha, prior_beta)
        ...
        ymax = max(np.max(kde_0(x)), np.max(kde_1(x)), np.max(prior_pdf))

        if observed_fraction_0 is not None:
            ax.annotate(
                f"Observed {group_0}\n{observed_fraction_0:.2f}",
                xy=(observed_fraction_0, ymax * 0.85),
                xytext=(observed_fraction_0, ymax * 1.05),
                ha="center",
                va="bottom",
                color=color_0,
        ...
        if observed_fraction_1 is not None:
            ax.annotate(
                ...
                xy=(observed_fraction_1, ymax * 0.85),
                xytext=(observed_fraction_1, ymax * 1.05),
                ha="center",
                va="bottom",
                color=color_1,
    ...
    def plot_confusion_matrix(
        self, *, prior_0: float = 0.5, savefig_kwargs: dict[str, Any] | None = None
    ) -> tuple[Figure, Axes]:
